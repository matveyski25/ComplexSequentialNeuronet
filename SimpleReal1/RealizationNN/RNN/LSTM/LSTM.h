...
namespace MyNN::RNN {
	...
	class LSTM : public BaseRNN<T> {
		public:
			...
			protected:
				struct ValuesForCompute : IComputeBlockRNN<T>::ValuesForCompute {
				...
				struct NState : ComputeBlockRNN<T>::NState {
					//input_state_n - [1 x I]
					...
					void setZero(const std::uint64_t & hidden_size) {
						this->n_cell_state = LinearAlgebra::BaseRowVector<T>::Zero(hidden_size);
						this->n_hidden_state = LinearAlgebra::BaseRowVector<T>::Zero(hidden_size);

						this->tmp_f = LinearAlgebra::BaseRowVector<T>::Zero(hidden_size);
						this->tmp_i = LinearAlgebra::BaseRowVector<T>::Zero(hidden_size);
						this->tmp_c_bar = LinearAlgebra::BaseRowVector<T>::Zero(hidden_size);
						this->tmp_o = LinearAlgebra::BaseRowVector<T>::Zero(hidden_size);

						this->tmp_Z = LinearAlgebra::BaseRowVector<T>::Zero(4 * hidden_size);
					}
				};
			...
			public:
				...
				public:
					void save(IComputeBlockNN<T> * compute_block_) override {
					...
					void setArgsForLoad(const typename IComputeBlockNN<T>::ILoader::ArgsLoader* args_) override {
				...
				public:
					void load(IComputeBlockNN<T>* compute_block_) override {
