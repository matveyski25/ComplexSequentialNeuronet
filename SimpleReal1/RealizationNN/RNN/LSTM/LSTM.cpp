...
namespace MyNN {
	namespace RNN {
			...
			const LinearAlgebra::BaseRowVector<T>& x_n)
		{
			...
			LinearAlgebra::BaseRowVector<T> new_c_n = n_state->tmp_f.array() * c_n_l.array() + n_state->tmp_i.array() * n_state->tmp_c_bar.array();
		...
		template<typename T>
		const typename LSTM<T>::DefaultComputeBlockOneH::ValuesForCompute* LSTM<T>::DefaultComputeBlockOneH::getValuesForCompute()
		{
			return this->values_for_compute.get();
		}

		template<typename T>
		void LSTM<T>::DefaultComputeBlockOneH::setValuesForCompute(const ValuesForCompute* values_for_compute_)
		{
