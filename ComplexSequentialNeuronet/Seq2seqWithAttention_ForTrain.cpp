...
/*Seq2SeqWithAttention_ForTrain::grads_Seq2SeqWithAttention Seq2SeqWithAttention_ForTrain::Backward(size_t Number_InputState, MatrixXld Y_True) {
	...
	Eigen::Index EE = this->encoder_->Forward->Common_Input_size;
	...
	Eigen::Index T = std::min(this->GetOutputs()[Number_InputState].rows(), Y_True.rows());
	Eigen::Index N = this->encoder_->Forward->Common_Input_states[Number_InputState].rows();
	...
	for (int64_t t = static_cast<int64_t>(T) - 1; t >= 0; t--) {
		...
		for (int64_t j = static_cast<int64_t>(N) - 1; j >= 0; j--) {
			...
			RowVectorXld h_j = this->encoder_->Forward->Common_Hidden_states[Number_InputState].row(j);
			RowVectorXld s_t_1;
			if (t > 0) {
			...
			for (int k = 0; k < N; ++k) {
				...
				RowVectorXld h_k = this->encoder_->Forward->Common_Hidden_states[Number_InputState].row(k);
				double dAlpha_k = dContext_t.dot(h_k);
